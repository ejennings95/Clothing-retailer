...
class ShoppingCartTotal
  ...
  def total_price(code, shoppingcart, stock = Stock.new)
    ...
    total_price_with_discount_check(code)
    @price = two_decimal_places(@price)
  end

  def total_price_with_discount_check(code, discountcodes = DiscountCodes.new)
    if code == nil
      @discount_price = two_decimal_places(@price)
    else
      @discount_price = two_decimal_places(@price - discountcodes.discount_check(code))
    end
  end
